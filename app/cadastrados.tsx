...
export default function Cadastrados() {
  ...
  const [activeTab, setActiveTab] = useState<"pets" | "criancas" | "idosos">("pets");
    ...
    try {
      ...
      petsSnapshot.forEach((docSnap) => {
        ...
        petsData.push({
          ...
          tipoUsuario: "pet",
          dataNascimento: data.dataNascimento || data.datanascimento || "Não informada",
      ...
      idososSnapshot.forEach((docSnap) => {
        ...
        idososData.push({
          ...
          tipoUsuario: "idoso",
          dataNascimento: data.dataNascimento || data.datanascimento || "Não informada",
          condicoesMedicas: data.condicoesMedicas || data.condicoes || "Não informada",
  ...
  const getCollectionName = () => {
    switch (activeTab) {
      ...
      case "criancas": return "crianca";
      case "idosos": return "idoso";
      default: return "";
    }
  };
  ...
  const getEmptyMessage = () => {
    switch (activeTab) {
      case "pets": return "Nenhum pet cadastrado";
      case "criancas": return "Nenhuma criança cadastrada";
      case "idosos": return "Nenhum idoso cadastrado";
      default: return "Nenhum membro cadastrado";
    }
  };
  ...
  const getEmptyIcon = () => {
    switch (activeTab) {
      ...
      case "criancas": return "heart";
      case "idosos": return "person";
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.tabsContainer}>
        ...
        >
          <Text style={[styles.tabText, activeTab === "criancas" && styles.activeTabText]}>
            Crianças
          </Text>
        </TouchableOpacity>
          ...
          {currentData.length === 0 ? (
            ...
            <>
              <Text style={styles.subtitle}>
                {currentData.length} {activeTab === "pets" ? 'pet' : activeTab === "criancas" ? 'criança' : 'idoso'}{currentData.length !== 1 ? 's' : ''} cadastrado{currentData.length !== 1 ? 's' : ''}
              </Text>
...
const styles = StyleSheet.create({
  ...
  petName: { fontSize: 18, fontWeight: "bold", color: "#0A2C5E", marginBottom: 4 },
  petDetails: { fontSize: 14, color: "#666666", marginBottom: 2 },
  actions: { flexDirection: "row" },
  actionButton: { width: 40, height: 40, borderRadius: 20, justifyContent: "center", alignItems: "center", borderWidth: 1 },
  deleteButton: { borderColor: "#FF3B30", backgroundColor: "#FFF0F0" },
  emptyState: { justifyContent: "center", alignItems: "center", paddingVertical: 100 },
  emptyStateTitle: { fontSize: 18, fontWeight: "bold", color: "#666666", marginTop: 16, marginBottom: 8 },
